...
const siteData = `<script>window.__SITE_DATA__=${jsonEscape({ tools, posts, pages })}</script>`;
...
for (const f of allHtml) {
  ...
  if (text.includes('id="navSearch"')) {
    const loader = `${siteData}<script src="${appSrc}"></script>`;
    text = text.replace(/<\/body>/i, `${loader}\n</body>`);
  }

  // AdSense insertion is marker-based and therefore idempotent.
  ...
  text = text.replace(/<div class="ad-reserved"[^>]*data-ad-placement="top"[^>]*><\/div>/gi, `<!-- EASYWORKTOOLS_AD_TOP_START -->${adTop}<!-- EASYWORKTOOLS_AD_TOP_END -->`);
  text = text.replace(/<div class="ad-reserved"[^>]*data-ad-placement="mid"[^>]*><\/div>/gi, `<!-- EASYWORKTOOLS_AD_MID_START -->${adMid}<!-- EASYWORKTOOLS_AD_MID_END -->`);
  text = text.replace(/<div class="ad-reserved"[^>]*data-ad-placement="bottom"[^>]*><\/div>/gi, `<!-- EASYWORKTOOLS_AD_BOTTOM_START -->${adBottom}<!-- EASYWORKTOOLS_AD_BOTTOM_END -->`);
